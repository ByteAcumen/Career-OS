...
import { InfoCard, PageHeader, SectionCard, StatCard, riseIn, sectionStagger } from "@/components/workspace/workspace-primitives";
    ...
    <motion.div variants={sectionStagger} initial="hidden" animate="show" className="grid gap-6">
      <motion.div variants={riseIn}>
        <PageHeader
          ...
          description="Settings now owns profile details, links, planner defaults, AI integrations, and account protection so the rest of the workspace can stay focused."
        />
      </motion.div>

      <motion.div variants={riseIn} className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Planner targets" value={`${settings.weekdayTaskTarget}/${settings.weekendTaskTarget}`} detail="Weekday and weekend task defaults." />
        <StatCard label="Weekly goals" value={settings.weeklyDsaTarget + settings.weeklyApplicationTarget + settings.weeklyBuildTarget} detail="Combined target checkpoints across DSA, apps, and builds." />
        <StatCard label="AI provider" value={settings.aiProvider} detail="Current active provider preference for AI features." />
        <StatCard label="Saved keys" value={Object.values(integrations.savedApiKeys).filter(Boolean).length} detail="Encrypted per-user AI credentials currently stored." />
      </motion.div>
        ...
        <motion.div variants={riseIn} className="grid gap-6">
          ...
          >
            <div className="grid gap-3">
              <InfoCard label="Primary goal" value={settings.primaryGoal || "No primary goal saved yet."} />
              <InfoCard label="Weekly theme" value={settings.weeklyTheme || "No weekly theme saved yet."} />
              <InfoCard label="Plan style" value={settings.planStyle || "No planning style saved yet."} />
              <InfoCard
                label="Profile completeness"
                value={settings.onboardingCompleted ? "Core onboarding is complete." : "Finish onboarding fields to improve personalization quality."}
              />
            </div>
          </SectionCard>
